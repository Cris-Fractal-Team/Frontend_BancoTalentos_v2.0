...
const NotaPanel = ({ children }: { children: ReactNode }) => (
  <p className="text-xs text-gray-400 mb-3 dark:text-slate-500">{children}</p>
);

/** Aviso de dato ausente dentro de un panel, con dónde se registra. */
const PanelVacio = ({ children }: { children: ReactNode }) => (
  <div className="flex items-start gap-2 p-3 rounded-lg bg-gray-50 text-xs text-gray-500 dark:bg-slate-800 dark:text-slate-400">
    <Info className="w-4 h-4 min-w-4 text-gray-400 mt-0.5 dark:text-slate-500" />
    <span>{children}</span>
  </div>
);
...
const BarraApilada = ({
  ...
  return (
    <div className="flex items-center gap-3 py-0.5">
      <span
        className="text-[13px] leading-[18px] font-medium text-gray-700 dark:text-slate-200"
      ...
      <span
        className="text-right text-[13px] font-semibold text-gray-900 tabular-nums dark:text-slate-50"
...
const FilaModalidad = ({
  ...
  return (
    <div className="py-1.5">
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {esLaDelTalento && (
            <>
              <br />
              <span className="text-[11px] leading-4 font-normal text-gray-400 dark:text-slate-500">
                la del talento
              </span>
            </>
          )}
      ...
      {banda.tieneDatos && banda.faltaTipoCambio && (
        <p
          className="text-[11px] leading-4 text-amber-700 mt-1 dark:text-amber-300"
      ...
      {comparable && !esLaDelTalento && (
        <p
          className="text-[11px] leading-4 text-gray-500 mt-1 dark:text-slate-400"
          style={{ marginLeft: SANGRIA_IZQ }}
        >
          Pide{" "}
        ...
        <Panel>
          ...
          >
            {marcas.map((marca) => (
              <span
                key={marca}
                className="absolute text-[11px] leading-4 text-gray-400 tabular-nums dark:text-slate-500"
                style={{
            ...
            children: (
              <Panel>
                ...
                <div className="flex items-baseline justify-between mb-2">
                  ...
                  <span className="text-sm font-semibold text-gray-900 tabular-nums dark:text-slate-50">
                ...
                />
                <div className="flex justify-between text-xs text-gray-400 mt-1 dark:text-slate-500">
                ...
                {equilibrioPrincipal !== null && (
                  <>
                    <div className="flex items-baseline justify-between gap-3 text-[13px] leading-5 py-1.5 mt-2 border-t border-gray-200 dark:border-slate-700">
                      <span className="text-gray-500 dark:text-slate-400">Pedir menos de</span>
                      <span className="font-semibold text-gray-900 tabular-nums dark:text-slate-50">
                        S/{" "}
                        {formatearMonto(equilibrioPrincipal * (1 - tolerancia))}{" "}
                        → Apto
                      </span>
                    </div>
                    <div className="flex items-baseline justify-between gap-3 text-[13px] leading-5 py-1.5 border-t border-gray-100 dark:border-slate-700">
                ...
                {toleranciaTope < TOLERANCIA_MAX && (
                  <p className="text-xs text-gray-400 mt-1.5 dark:text-slate-500">
  ...
  return (
    <>
      ...
      <div className="px-6 pb-6">
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-4 py-4 border-b border-gray-100 dark:border-slate-700">
          <Celda
            rotulo="Pide (bruto)"
        ...
        {!factorPlanilla.configurado && (
          <div className="flex items-start gap-2 p-3 rounded-lg border border-amber-200 bg-amber-50 text-sm text-amber-900 mt-4 dark:border-amber-500/30 dark:bg-amber-500/10">
        ...
        {requiereTipoCambio && (
          <div className="flex items-center gap-3 p-4 rounded-lg border border-amber-200 bg-amber-50 mt-4 dark:border-amber-500/30 dark:bg-amber-500/10">
            <label
